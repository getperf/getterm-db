...
- **CTRL+SHIFT+J**  
    → ミュート機能のオン・オフを切り替えます。ミュートオンの場合はターミナルの記録をノートに書き込みません。
...
- ターミナルおよびノートブックを操作する際は、以下のショートカットキーも併用してください：
    ...
    - CTRL-SHIFT-J : ターミナルのノートへの書き込みを抑止するミュート機能の切り替えを行います。
...
- **`getterm-db.toggleMute`**  
    ...
    **カテゴリー:** Notebook  
    **説明:**  
    ノートを開くと、ステータスバーに **Mute (ミュート)** または **Unmute (ミュート解除)** が表示され、本コマンドを実行するか、ラベルをクリックすることで切り替えが可能です。
	ミュートがオン (Muted) の場合、ノートブックへの出力や記録が抑制され、端末の記録内容が書き込まれません。
...
このプロジェクトでは、依存関係の脆弱性を定期的に `npm audit` や Snyk を使用してチェックし、必要に応じて依存パッケージのアップデートを行っています。GitHub の Dependabot も有効にしており、検出された脆弱性については迅速に対応する方針です。
