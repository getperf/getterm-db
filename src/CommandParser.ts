...
export class CommandParser {
    ...
    static splitBufferByCommandSequence(buffer: string): {
        ...
        const outputBuffer = buffer.slice(
            indexCommandC + osc633CommandC.length,
        );
        ...
        Logger.debug(
            `split the console buffer into the command : ${JSON.stringify(commandBuffer)}  and output : ${JSON.stringify(outputBuffer)}`,
        );
        return {
            commandBuffer,
            outputBuffer,
    ...
    static selectCompleteCommand(
        startCommandText: string | undefined,
        eCommandText: string,
    ): string {
        Logger.info(
            `choose complete command : ${startCommandText} and ${eCommandText}`,
        );
        if (!startCommandText) {
            return eCommandText;
        }
    ...
    static async parse(buffer: string, options: ParserOptions = {}): Promise<ParsedCommand> {
        ...
        while (
            (match = oscRegex.exec(
                parts.commandBuffer + parts.outputBuffer,
            )) !== null
        ) {
        ...
        parsedCommand.command = this.selectCompleteCommand(
            commandText,
            eCommandText,
        );
