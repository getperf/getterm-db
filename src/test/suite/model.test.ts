...
suite("Database Models", function () {
    ...
    test("Get All Commands by Session ID and Delete All Commands by Session ID", async function () {
        ...
        assert.strictEqual(
            deletedCommands.length,
            0,
            "All commands were deleted.",
        );
    });
    ...
    test("should update command file modification fields correctly", async () => {
        // Insert test data
        const sessionId = await Session.create(
            "test_profile",
            "/path/to/exe",
        ...
        const command = await Command.getById(commandId);
        assert.strictEqual(
            command.file_operation_mode,
            updateMode,
            "File update mode should be updated",
        );
        assert.strictEqual(
            command.command_access_file,
            commandAccessFile,
            "Update file path should be updated",
        );
        assert.strictEqual(
            command.download_file,
            downloadFile,
            "Download file path should be updated",
        );
    });
    ...
    test("should throw an error if no command is found", async () => {
        ...
        try {
            ...
            if (err instanceof Error) {
                ...
                assert.fail("Caught error is not an instance of Error");
            }
        }
    });
});
